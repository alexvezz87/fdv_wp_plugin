...
if($carrello->isCarrelloInizializzato() && !$carrello->isCarrelloEmpty()){
    ...
    while($i < $carrello->getDimensioneCarrello()){
        ...
        if($temp_articolo_ordine->getCassettaPersonalizzata() == 0){
            ...
            if($tabella == 0){
                ...
                _e(     '<input class="remove-button" style="float:right;" type="submit" value="" title="Rimuovi dal carrello" name="elimina-da-carrello">');
                _e(     $foto_da_inserire);
                ...
                //ottengo le coordinate del
                
                $coord = getCoordinateSprite($fv_info->ID_Foto);
                
                if(image_exists($foto_fv)){ $foto_da_inserire = '<img src="'.$foto_fv.'">'; }
                else{ $foto_da_inserire = '<img src="'.$path.'non-disponibile.jpg" >';}
            ...
            if($scritto == false && $k == $temp_articolo_ordine->getCassettaPersonalizzata()){
                   _e('<div class="prodotto visualizza-cassetta-personalizzata ');
                    if( ($i  % 2 == 0)){_e('pari"');} else{_e('dispari"');}
                   _e('><form action="'.$url.'cp='.$temp_articolo_ordine->getCassettaPersonalizzata().'" method="post"><input class="remove-button" style="float:right;" type="submit" title="Rimuovi dal carrello" value="" name="elimina-cp"><div class="clear" style="font-weight:bold">1 x Cassetta Personalizzata '.$k.'</div></form>');
   ...
   _e('<div style="margin-top:10px; margin-bottom:15px;"><input class="aggiungi" style="float:right" onClick="location.href=\'http://alexsoluzioniweb.it/progetti/fdv/fai-la-spesa/step-2/\'" type="button" value="CONTINUA >> "></div><div class="clear"></div>');
    _e('</div>');
    _e('</div>');
 }
